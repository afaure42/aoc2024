	...
	for pos_vec in antennas.values() {
		for pos_a in pos_vec {
			for pos_b in pos_vec {
				...
				let antinode_a: Vector2<i32> = pos_a + (pos_a - pos_b);
				let antinode_b: Vector2<i32> = pos_b + (pos_b - pos_a);

				if valid_pos(antinode_a, y_max, x_max) {antinodes.insert(antinode_a);}
				if valid_pos(antinode_b, y_max, x_max) {antinodes.insert(antinode_b);}
